...
export default function ConfiscateStaffDialog() {
  ...
  const handleConfiscate = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: `Reset ${okCount} of ${list.length} Crest Financial staff passwords. CSV downloaded.`,
      });
    } catch (e: unknown) {
  ...
  const downloadCsv = (list: ConfiscationResult[]) => {
    ...
    a.href = url;
    a.download = `crest-financial-staff-confiscated-${new Date().toISOString().replace(/[:.]/g, '-')}.csv`;
    ...
    <Dialog open={open} onOpenChange={(v) => { setOpen(v); if (!v) reset(); }}>
      <DialogTrigger asChild>
        <Button variant="destructive">
          ...
          Confiscate Crest Financial Staff
        </Button>
      </DialogTrigger>
      <DialogContent className="sm:max-w-3xl">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            Regime Change — Confiscate Crest Financial Staff
          </DialogTitle>
          <DialogDescription>
            This will immediately reset the passwords of every Crest Financial staff member
        ...
        {!results && (
          <div className="space-y-4 py-2">
            <div className="p-3 bg-destructive/10 border border-destructive/20 rounded-md text-sm">
              All existing Crest Financial accounts (staff + clients) have already been hidden
